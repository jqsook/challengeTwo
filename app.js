...
function update() {
  ...
  document.getElementById(
    "id-axePrice"
  ).innerHTML = `${clickAxes.pickaxe.price}`;
  document.getElementById(
    "id-tractr"
  ).innerHTML = `${clickTractors.tractor.quantity}`;
  document.getElementById(
    "id-trctPrice"
  ).innerHTML = `${clickTractors.tractor.price}`;
  ...
  document.getElementById(
    "id-trkPrice"
  ).innerHTML = `${autoTrucks.truck.price}`;
}
// -----------------Pick Axe---------------- //
function buyPickAxe() {
...
//  -----------  Boat   ----------------------//
function buyBoat() {
  if (cheese > autoBoats.boat.price && autoBoats.boat.quantity <= 4) {
    //Limited to 4 for the switch cases- I was trying to get the for in loops to work, but no go.
